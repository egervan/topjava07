...
public class UserMealsUtil {
    ...
    public static void main(String[] args) {
        List<UserMealWithExceed> filteredMealsWithExceeded = getFilteredWithExceeded(MEAL_LIST, LocalTime.of(7, 0).atDate(LocalDate.MIN), LocalTime.of(12, 0).atDate(LocalDate.MAX), DEFAULT_CALORIES_PER_DAY);
        filteredMealsWithExceeded.forEach(System.out::println);

        System.out.println(getFilteredWithExceededByCycle(MEAL_LIST, LocalTime.of(7, 0).atDate(LocalDate.MIN), LocalTime.of(12, 0).atDate(LocalDate.MAX), DEFAULT_CALORIES_PER_DAY));
    }
    ...
    public static List<UserMealWithExceed> getFilteredWithExceeded(Collection<UserMeal> mealList, LocalDateTime start, LocalDateTime end, int caloriesPerDay) {
        ...
        return mealList.stream()
                .filter(um -> TimeUtil.isBetween(um.getDateTime(), start, end))
                .map(um -> createWithExceed(um, caloriesSumByDate.get(um.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(Collectors.toList());
    }

    public static List<UserMealWithExceed> getFilteredWithExceededByCycle(List<UserMeal> mealList, LocalDateTime start, LocalDateTime end, int caloriesPerDay) {
        ...
        mealList.forEach(meal -> {
            ...
            if (TimeUtil.isBetween(dateTime, start, end)) {
